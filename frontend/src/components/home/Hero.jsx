...
export default function Hero() {
  ...
  const heroImage = cms?.hero?.backgroundImage || "/images/hotel-yashdeep/hotel main.png";
  const headline = cms?.hero?.headline;
  const subheadline = cms?.hero?.subheadline || "Authentic Maharashtrian family dining, fresh daily dam catch, and a chilled beer bar on the Yermala highway.";
  const ctaText = cms?.hero?.ctaText || "Reserve a Table";
  const ctaLink = cms?.hero?.ctaLink || "/reservations";
      ...
      <div className="relative container-luxe min-h-[100svh] flex flex-col">
        <div className="flex-1 grid lg:grid-cols-[1.4fr_1fr] gap-8 lg:gap-10 items-end pt-28 sm:pt-32 pb-12 sm:pb-16">
          ...
          >
            ...
            >
              <Link to={ctaLink} className="btn-gold justify-center shadow-md hover:shadow-lg" data-testid="hero-reserve-btn">
                {ctaText} <ArrowRight size={16} />
              </Link>
              <a href={phoneHref} className="inline-flex items-center justify-center gap-2 rounded-full border border-background/30 px-6 sm:px-7 py-3 sm:py-3.5 text-xs sm:text-sm text-background hover:border-secondary hover:text-secondary transition-colors duration-500" data-testid="hero-call-btn">
                <Phone size={14} /> {phone}
              </a>
            </motion.div>
          </motion.div>
